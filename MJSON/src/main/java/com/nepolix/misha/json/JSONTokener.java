...
/**
 * Parses a JSON (<a href="http://www.ietf.org/rfc/rfc4627.txt">RFC 4627</a>) encoded string into the corresponding object. Most clients of this class will use only need the {@link
 * #JSONTokener(String) constructor}
 * and {@link #nextValue} method. Example usage: <pre>
 * String com.nepolix.misha.json = "{"
 *         + "  \"query\": \"Pizza\", "
 *         + "  \"locations\": [ 94043, 90210 ] "
 *         + "}";
 * <p>
 * JSONObject object = (JSONObject) new JSONTokener(com.nepolix.misha.json).nextValue();
 * String query = object.getString("query");
 * JSONArray locations = object.getJSONArray("locations");</pre>
 * <p>
 * <p>This parser is lenient. A successful parse does not necessarily indicate that the input string is valid JSON.
 * <p>
 * <p>Each tokener may be used to parse a single JSON string. Instances of this class are not thread safe. Although this class is nonfinal, it was not designed for inheritance and should not be
 * subclassed. In particular, self-use by overridable methods is not specified. See <i>Effective Java</i> Item 17, "Design and Document or inheritance or else prohibit it" for further information.
 */
class JSONTokener
{
	 ...
	 /**
		* Advances the position until it is beyond the current comment. The opening slash '/' should have already been read, and character at the current position be an asterisk '*' for a C-style comment
		* or a slash '/' for an end-of-line comment.
		*
		* @throws JSONException
		* 				if a C-style comment was not terminated.
		*/
	 private
	 void skipComment ( )
					 throws
	 ...
	 /**
		* Returns the string up to but not including {@code quote}, unescaping any character escape sequences encountered along the way. The opening quote should have already been read. This consumes the
	 ...
	 /**
		* Unescapes the character identified by the character or characters that immediately follow a backslash. The backslash '\' should have already been read. This supports both unicode escapes
		* "u000A"
	 ...
	 /**
		* Reads a sequence of values and the trailing closing brace ']' of an array. The opening brace '[' should have already been read. Note that "[]" yields an empty array, but "[,]" returns a
     ...
     */
	 ...
	 /**
		* Returns the next character that is not whitespace and does not belong to a comment. If the input is exhausted before such a character can be found, the null character '\0' is returned. The
		* return $value of this method is ambiguous for JSON strings that contain the character '\0'.
		*/
	 public
	 char nextClean ( )
					 throws
	 ...
	 /**
		...
		* <p>
		* <p>The returned string shares its backing character array with this tokener's input string. If a reference to the returned string may be held indefinitely, you should use {@code new
		* String(result)} to copy it first to avoid memory leaks.
		*
		* @throws JSONException
