...
/**
 * A dense indexed sequence of values. Values may be any mix of {@link JSONObject JSONObjects}, other {@link JSONArray JSONArrays}, Strings, Booleans, Integers, Longs, Doubles, {@code null} or {@link
 * JSONObject#NULL}. Values may not be {@link Double#isNaN() NaNs}, {@link Double#isInfinite() infinities}, or of any type not listed here.
 * <p>
 * <p>{@code JSONArray} has the same type coercion behavior and optional/mandatory accessors as {@link JSONObject}. See that class' documentation for details.
 * <p>
 * <p><strong>Warning:</strong> this class represents null in two incompatible ways: the standard Java {@code null} reference, and the sentinel $value {@link JSONObject#NULL}. In particular, {@code
 * get} fails if the requested index holds the null reference, but succeeds if it holds {@code JSONObject.NULL}.
 * <p>
				...
				implements Iterable
{
	 ...
	 /**
		...
		*
		* @param value
		* 				a {@link JSONObject}, {@link JSONArray}, String, Boolean, Integer, Long, Double, {@link JSONObject#NULL}, or {@code null}. May not be {@link Double#isNaN() NaNs} or {@link
		* 				Double#isInfinite() infinities}. Unsupported values are not permitted and will cause the array to be in an inconsistent state.
		*
		* @return this array.
		*/
	 public
	 ...
	 /**
		...
		*
		* @throws JSONException
		* 				if this array has no $value at {@code index}, or if that $value is the {@code null} reference. This method returns normally if the $value is {@code JSONObject#NULL}.
		*/
	 public
	 ...
	 /**
		* Returns a new object whose values are the values in this array, and whose names are the values in {@code names}. Names and values are paired up by index from 0 through to the shorter array's
		* length. Names that are not strings will be coerced to strings. This method returns null if either array is empty.
		*/
	 public
	 ...
	 /**
		* Returns a new string by alternating this array's values with {@code separator}. This array's string values are quoted and have their special characters escaped. For example, the array
		* containing
		* the strings '12" pizza', 'taco' and 'soda' joined on '+' returns this:
			...
			try
			{
				 values.forEach ( v -> {
						...
						else
						{
							 ...
							 {
									cloneArray.put ( ( ( JSONObject ) v ).clone ( ) );
							 }
							 else
							 {
									if ( v.getClass ( ).equals ( JSONArray.class ) )
									{
										 cloneArray.put ( ( ( JSONArray ) v ).clone ( ) );
									}
									else
									{
										 cloneArray.put ( v );
