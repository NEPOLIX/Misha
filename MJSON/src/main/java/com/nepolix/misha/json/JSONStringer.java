...
/**
 * Implements {@link JSONObject#toString} and {@link JSONArray#toString}. Most application developers should use those methods directly and disregard this
 * API. For example:<pre>
 * JSONObject object = ...
 * String com.nepolix.misha.json = object.toString();</pre>
 * <p>
 * <p>Stringers only encode well-formed JSON strings. In particular: <ul> <li>The stringer must have exactly one top-level array or object. <li>Lexical scopes must be balanced: every call to {@link
 * #array} must have a matching call to {@link #endArray} and every call to {@link #object} must have a matching call to {@link #endObject}. <li>Arrays may not contain keys (property names).
 * <li>Objects must alternate keys (property names) and values. <li>Values are inserted with either literal {@link #value(Object) $value} calls, or by nesting arrays or objects. </ul> Calls that would
 * result in a malformed JSON string will fail with a {@link JSONException}.
 * <p>
 * <p>This class provides no facility for pretty-printing (ie. indenting) output. To encode indented output, use {@link JSONObject#toString(int)} or {@link JSONArray#toString(int)}.
 * <p>
 * <p>Some implementations of the API support at most 20 levels of nesting. Attempts to create more than 20 levels of nesting may fail with a {@link JSONException}.
 * <p>
 * <p>Each stringer may be used to encode a single top level $value. Instances of this class are not thread safe. Although this class is nonfinal, it was not designed for inheritance and should not be
...
class JSONStringer
{
	 ...
	 /**
		...
		*/
	 JSONStringer close ( Scope empty ,
												Scope nonempty ,
												String closeBracket )
					 throws
					 JSONException
	 ...
	 /**
		...
		* <p>
		* <p>If invoked with unterminated arrays or unclosed objects, this method's return $value is undefined.
		* <p>
		* <p><strong>Warning:</strong> although it contradicts the general contract of {@link Object#toString}, this method returns null if the stringer contains no data.
		*/
	 @Override
	 public
	 String toString ( )
