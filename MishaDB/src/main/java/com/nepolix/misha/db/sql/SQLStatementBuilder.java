...
package com.nepolix.misha.db.sql;
					 ...
					 MishaSQLFormatException
	 {
			...
			try
			{
				 ...
				 if ( type.equals ( "FIND" ) )
				 {
						...
						if ( sql.has ( "LIMIT" ) ) builder.append ( " LIMIT " ).append ( sql.getInt ( "LIMIT" ) );
						if ( sql.has ( "OFFSET" ) ) builder.append ( " OFFSET " ).append ( sql.getInt ( "OFFSET" ) );
				 }
					 ...
					 MishaSQLFormatException
	 {
			...
			for ( String key : keys )
			{
				 ...
				 else
				 {
						...
						try
						{
							 if ( fieldChain != null ) throw new MishaSQLFormatException ( "fieldChain JSON inside another fieldChain JSON is " + "illegal" );
